...
class WorstPlayersPage extends StatefulWidget {
  const WorstPlayersPage({super.key});

  @override
  WorstPlayersPageState createState() => WorstPlayersPageState();
}

class WorstPlayersPageState extends State<WorstPlayersPage> {
  String getWorstPlayers = 'http://10.0.2.2:5000/worst';

  getPlayers(String position) async {
    try {
      final uri = Uri.parse(getWorstPlayers);
      final response = await http.get(uri);
      if (response.statusCode == 200) {
        final jsonResponse = json.decode(response.body);
        final performers = jsonResponse[position];
        Navigator.pushNamed(context, '/worstP', arguments: { performers: { ...performers, position}});
      } else {
